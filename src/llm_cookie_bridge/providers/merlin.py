...
class MerlinProvider(BaseProvider):
    """Merlin AI provider (getmerlin.in).

    Authentication: session cookies from https://www.getmerlin.in.

    Example::

        bridge = LLMCookieBridge.create(
            "merlin",
            cookie_header=os.environ["MERLIN_COOKIE_HEADER"],
        )
    ...
    * ``chat_id`` – Continue an existing conversation.
    ...
    default_base_url = "https://www.getmerlin.in"
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        if not dict(self.client.cookies):
            raise AuthenticationError(
                "Merlin requires session cookies. "
                "Log in at https://www.getmerlin.in and export the cookie header."
            )

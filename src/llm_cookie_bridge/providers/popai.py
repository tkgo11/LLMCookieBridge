...
class PopAIProvider(BaseProvider):
    """PopAI provider (popai.pro).

    Authentication: session cookies from https://popai.pro.

    Example::

        bridge = LLMCookieBridge.create(
            "popai",
            cookie_header=os.environ["POPAI_COOKIE_HEADER"],
        )
    ...
    * ``chat_id`` – Continue an existing conversation.
    ...
    provider_name = "popai"
    default_base_url = "https://popai.pro"
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        if not dict(self.client.cookies):
            raise AuthenticationError(
                "PopAI requires session cookies. "
                "Log in at https://popai.pro and export the cookie header."
            )

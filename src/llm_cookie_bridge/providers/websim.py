...
class WebSimProvider(BaseProvider):
    """WebSim.ai provider — AI website/web-app generator chat.

    Authentication: session cookies or Bearer token from https://websim.ai.

    Example::

        bridge = LLMCookieBridge.create(
            "websim",
            auth_token=os.environ["WEBSIM_TOKEN"],
        )

    Provider-specific chat options:

    * ``chat_id`` – Continue an existing project.
    """

    provider_name = "websim"
    default_base_url = "https://websim.ai"

    _CHAT_PATH = "/api/v1/chat/completions"

    DEFAULT_MODEL = "websim"
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        raise AuthenticationError(
            "WebSim requires an auth_token. "
            "Log in at https://websim.ai and copy the Bearer token from a "

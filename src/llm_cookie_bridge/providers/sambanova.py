...
class SambanovaProvider(BaseProvider):
    """SambaNova Cloud provider (api.sambanova.ai / cloud.sambanova.ai).

    SambaNova Cloud offers fast inference for frontier open-source models
    (Llama, DeepSeek, QwQ) on their custom RDU hardware.  The API is
    OpenAI-compatible.

    Authentication: A SambaNova API key.

    1. Sign up at https://cloud.sambanova.ai
    2. Go to **API Access** and generate an API key.
    ...
    * ``model`` – Model name, defaults to ``"Meta-Llama-3.3-70B-Instruct"``.
      Others: ``"DeepSeek-R1"``, ``"QwQ-32B"``, ``"Meta-Llama-3.1-405B-Instruct"``.
    ...
    * ``max_tokens`` – Max output tokens (default 1024).
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        raise AuthenticationError(
            "SambaNova requires an API key. "
            "Sign up at https://cloud.sambanova.ai and create an API key."
        )

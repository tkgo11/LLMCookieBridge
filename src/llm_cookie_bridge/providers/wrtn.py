...
class WrtnProvider(BaseProvider):
    """Wrtn provider (wrtn.ai) — Korean AI assistant aggregator.

    Authentication: session cookies from https://wrtn.ai.

    Example::

        bridge = LLMCookieBridge.create(
            "wrtn",
            cookie_header=os.environ["WRTN_COOKIE_HEADER"],
        )
    ...
    * ``chat_id`` – Continue an existing conversation.
    ...
    provider_name = "wrtn"
    default_base_url = "https://wrtn.ai"
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        if not dict(self.client.cookies):
            raise AuthenticationError(
                "Wrtn requires session cookies. "
                "Log in at https://wrtn.ai and export the cookie header."
            )
